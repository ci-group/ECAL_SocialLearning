...
public class NEATEvolution extends Evolution {
	...
	public int islandModel = jBotEvolver.getArguments().get("--population").getArgumentAsInt("islandModel");
	public double communicationChange = jBotEvolver.getArguments().get("--population").getArgumentAsDouble("communicationChange");
	...
	public void executeEvolution() {
		...
		while (!population[numberPopulations-1].evolutionDone() && executeEvolution) {
			...
			for(p=0;p<numberPopulations;p++){
				...
				try {
					...
					// SET p to zero for island model??
					if(islandModel==1){
						indexPopulation.write(0+ "\n");
			...
			if(socialLearning==1){
				...
				int ind;
				int worstid=0;
				double[][] sortPopulation = new double[population[p].populationSize][2];
				
				for(ind=0; ind<population[p].populationSize;ind++){
					sortPopulation[ind][0] = ind;
					sortPopulation[ind][1] = fullPopulation[ind].fitness();
				}

				
				java.util.Arrays.sort(sortPopulation, new java.util.Comparator<double[]>() {
				    public int compare(double[] a, double[] b) {
				        return Double.compare(a[1], b[1]);
				    }
				});
				
				//System.out.println("zero value" + sortPopulation[0][0]);

				// this part is: exchange with random other robot
				/*Random r = new Random();
				int Result = r.nextInt(numberPopulations);
				while(Result ==p){
					Result = r.nextInt(numberPopulations);
				}*/
				// this part is used for multiple robot tuning: probability to talk to other robots. 
				...
				int pops;
				int chromstaken=0;
				for(pops=0; pops<numberPopulations ;pops++){
					//System.out.println("pops" + pops + "p" + p);
					if(p!=pops){ //skip yourself
						if(r.nextDouble() <= communicationChange){
							//System.out.println("communicate");
							//System.out.println("worst" + fullPopulation[(int)(sortPopulation[chromstaken][0])].fitness() + "best" + bestChromosomePop[pops].fitness() );
							if(fullPopulation[(int)(sortPopulation[chromstaken][0])].fitness() < bestChromosomePop[pops].fitness()){
								//System.out.println("replace" + sortPopulation[chromstaken][0]);
								fullPopulation[(int)(sortPopulation[chromstaken][0])] = bestChromosomePop[pops];
								chromstaken+=1;
							}
						}
					}
				...
				//System.out.print("get from population" + Result + "now population" + p );
				//if(worstind.fitness() < bestChromosomePop[Result].fitness()){
				//fullPopulation[worstid]= bestChromosomePop[Result];
				//}
			}
		...
		File f1 = new File(result+"/_carriedPuck"+Integer.toString(p)+".log");
	...
	protected void configureDescriptor(NEATGADescriptor descriptor, Arguments args, Population population) {
		...
		// int specieAgeThreshold = args.getArgumentAsIntOrSetDefault("specieAgeThreshold", 0.8);
		...
		// int specieYouthThreshold = args.getArgumentAsIntOrSetDefault("specieYouthThreshold", 0.1);
